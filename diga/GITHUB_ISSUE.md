# GitHub issue body — ready to file

**Suggested target repo:** `ma3u/TwoBreath-app` (the iOS app repo). Alternative: open in `ma3u/TwoBreath` if you'd rather keep the certification thread next to the website.

**Suggested title:**
```
DiGA / BSI TR-03161 — evaluate TwoBreath as worked example for certification process improvement
```

**Suggested labels:** `epic`, `compliance`, `diga`, `bsi`, `tr-03161`

**Suggested filing command** (run from inside one of the repo subdirs):

```bash
gh issue create \
  --repo ma3u/TwoBreath-app \
  --title "DiGA / BSI TR-03161 — evaluate TwoBreath as worked example for certification process improvement" \
  --label "epic,compliance,diga" \
  --body-file ../diga/GITHUB_ISSUE_BODY.md
```
...
## ⬇ Issue body — copy from below this line ⬇
...
Planning artefacts live under [`diga/`](../diga/) in this tree:

- [`diga/PLANNING.md`](../diga/PLANNING.md) — goal framing, eligibility eval, phases
- [`diga/COMPLIANCE_MATRIX.md`](../diga/COMPLIANCE_MATRIX.md) — regulation ↔ proof ↔ tool ↔ result
- [`diga/MEMORY.md`](../diga/MEMORY.md) — markdown-first memory + run ledger
