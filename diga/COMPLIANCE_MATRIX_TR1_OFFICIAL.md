...
| --- | --- | --- | --- | --- | --- |
| [O.Purp_1](regulations/markdown/BSI-TR-03161-1.md#o-purp-1) | Informationspflicht des Herstellers | CHECK | M + D | 🟡 | PATCH-PENDING — Patch § 5 dokumentiert in patches/PATCHES.md, NOCH NICHT im TwoBreath-app eingespielt. Originalbewertung (mit Patch): App-Store-Beschreibung + Info.plist + In-App-Erstinformations-Bildschirm (Patch § 5) + Datenschutzkonzept (concepts/01-datenschutzkonzept.md). PrivacyInfo.xcprivacy maschinenlesbar. |
...
| [O.Purp_3](regulations/markdown/BSI-TR-03161-1.md#o-purp-3) | Einholung einer | CHECK | M + D | 🟡 | PATCH-PENDING — Patch § 5 + § 9 dokumentiert in patches/PATCHES.md, NOCH NICHT im TwoBreath-app eingespielt. Originalbewertung (mit Patch): iOS-vermittelte System-Dialoge plus In-App-Erstinformations-Texte (Patch § 5). Tatsache der Einwilligung wird per ConsentTracker (Patch § 9) protokolliert. |
...
| [O.Source_3](regulations/markdown/BSI-TR-03161-1.md#o-source-3) | Keine sensiblen Daten in Meldungen. | CHECK | D | 🟡 | PATCH-PENDING — Patch § 10 (SwiftLint-Custom-Regel) dokumentiert in patches/PATCHES.md, NOCH NICHT im TwoBreath-app eingespielt. Originalbewertung (mit Patch): Secure-Coding-Standards § 2 erzwingt privacy-Marker bei jeder os.Logger-Interpolation. SwiftLint-Custom-Regel `os_log_privacy_marker` (Patch § 10) blockt Verstöße in CI. `print(...)` in Produktiv-Pfaden verboten. |
...
| [O.Source_9](regulations/markdown/BSI-TR-03161-1.md#o-source-9) | Aktivierung von modernen | CHECK | D | 🟡 | PATCH-PENDING — Patch § 8 (Symbol-Stripping) dokumentiert in patches/PATCHES.md, NOCH NICHT im TwoBreath-app eingespielt. Originalbewertung (mit Patch): Apple-Toolchain-Defaults aktiv. Symbol-Stripping in Release-Konfiguration explizit gesetzt (Patch § 8: STRIP_INSTALLED_PRODUCT, STRIP_SWIFT_SYMBOLS, DEAD_CODE_STRIPPING). |
...
| [O.Data_10](regulations/markdown/BSI-TR-03161-1.md#o-data-10) | Keine Aufzeichnungen bei der | EXAMINE | D | 🟡 | PATCH-PENDING — Patch § 3 dokumentiert in patches/PATCHES.md, NOCH NICHT im TwoBreath-app eingespielt. Originalbewertung (mit Patch): Pairing-Code-Eingabe mit `.textContentType(.oneTimeCode)` + `.privacySensitive()` + `.autocorrectionDisabled()` (Patch § 3). |
| [O.Data_11](regulations/markdown/BSI-TR-03161-1.md#o-data-11) | Kein Export sensibler Daten in | CHECK | D | 🟡 | PATCH-PENDING — Patch § 4 dokumentiert in patches/PATCHES.md, NOCH NICHT im TwoBreath-app eingespielt. Originalbewertung (mit Patch): Sensible Anzeigen mit `.textSelection(.disabled)` und `.privacySensitive()` markiert (Patch § 4). |
...
| [O.Data_13](regulations/markdown/BSI-TR-03161-1.md#o-data-13) | Keine Speicherung des | CHECK | D | 🟡 | PATCH-PENDING — Patch § 2 dokumentiert in patches/PATCHES.md, NOCH NICHT im TwoBreath-app eingespielt. Originalbewertung (mit Patch): AppSwitcherShield-ViewModifier (Patch § 2) verdeckt sensible Anzeigen bei Übergang in den Hintergrund. Anwendung auf alle HealthKit-anzeigenden Views. |
...
| [O.Data_17](regulations/markdown/BSI-TR-03161-1.md#o-data-17) | Möglichkeit zum Löschen oder | EXAMINE | D | 🟡 | PATCH-PENDING — Patch § 7 dokumentiert in patches/PATCHES.md, NOCH NICHT im TwoBreath-app eingespielt. Originalbewertung (mit Patch): DataEraseService + Settings-UI mit Confirmation-Dialog (Patch § 7) löscht SessionLog, ConsentLog, PartnerPairing und entfernt App-eigene HealthKit-Mindful-Minutes. |
...
| ID | Kurzfassung (TR-03161-1 § 4.3) | Prüftiefe | Class | TwoBreath | Begründung / Quelle |
| --- | --- | --- | --- | --- | --- |
| [O.Plat_1](regulations/markdown/BSI-TR-03161-1.md#o-plat-1) | Geräteschutz für die Nutzung der | CHECK | M | 🟡 | PATCH-PENDING — Patch § 5 dokumentiert in patches/PATCHES.md, NOCH NICHT im TwoBreath-app eingespielt. Originalbewertung (mit Patch): Sicherheits-Empfehlungs-Bildschirm (Patch § 5) klärt explizit über Geräte-Sperre / Face-/Touch-ID auf — Erstes Bullet im SecurityInfoView. |
...
| [O.Plat_9](regulations/markdown/BSI-TR-03161-1.md#o-plat-9) | Entfernung von sensiblen Daten bei | EXAMINE | D | 🟡 | PATCH-PENDING — Patch § 2 dokumentiert in patches/PATCHES.md, NOCH NICHT im TwoBreath-app eingespielt. Originalbewertung (mit Patch): AppSwitcherShield-ViewModifier (Patch § 2) maskiert sensible Inhalte bei Übergang in den Hintergrund. |
...
| [O.Plat_13](regulations/markdown/BSI-TR-03161-1.md#o-plat-13) | Information des Nutzers über | CHECK | M | 🟡 | PATCH-PENDING — Patch § 5 dokumentiert in patches/PATCHES.md, NOCH NICHT im TwoBreath-app eingespielt. Originalbewertung (mit Patch): SecurityInfoView (Patch § 5) im Settings-Bildschirm enthält Sicherheits-Empfehlungen, Verarbeitungs-Übersicht, Verlauf-Verlinkung, Vulnerability-Disclosure-Link. |
...
| ID | Kurzfassung (TR-03161-1 § 4.3) | Prüftiefe | Class | TwoBreath | Begründung / Quelle |
| --- | --- | --- | --- | --- | --- |
| [O.Resi_1](regulations/markdown/BSI-TR-03161-1.md#o-resi-1) | Informationen zum sicheren | CHECK | M | 🟡 | PATCH-PENDING — Patch § 5 dokumentiert in patches/PATCHES.md, NOCH NICHT im TwoBreath-app eingespielt. Originalbewertung (mit Patch): Best-Practice-Empfehlungen im SecurityInfoView (Patch § 5) — gleicher Bildschirm wie O.Plat_13. |
...
| [O.Resi_3](regulations/markdown/BSI-TR-03161-1.md#o-resi-3) | Erkennung und Unterbindung des | CHECK | D | 🟡 | PATCH-PENDING — Patch § 1 dokumentiert in patches/PATCHES.md, NOCH NICHT im TwoBreath-app eingespielt. Originalbewertung (mit Patch): `assertNotDebugged()`-Funktion via `sysctl(KERN_PROC, …, P_TRACED)` (Patch § 1), aufgerufen in App-Init. Wirkt nur in Release-Builds. |
...
| [O.Resi_8](regulations/markdown/BSI-TR-03161-1.md#o-resi-8) | Umsetzung von Maßnahmen gegen | EXAMINE | D | 🟡 | PATCH-PENDING — Patch § 8 dokumentiert in patches/PATCHES.md, NOCH NICHT im TwoBreath-app eingespielt. Originalbewertung (mit Patch): Apple-Toolchain-Defaults + scharf gesetztes Symbol-Stripping in Release (Patch § 8). Trade-off-Begründung gegen kommerzielle Obfuscation in concepts/08-resilienz-haertungskonzept.md § 8 dokumentiert. |
